...
/**
 * AFTER (post-regression) benchmark — braintrust@3.2.0
 *
 * This version uses sequential chunk uploads (while loop with await per chunk),
 * resulting in significantly slower flush times compared to the pre-regression version.
 *
...
const LABEL = "AFTER (post-regression)";
const RESULTS_DIR = path.resolve(import.meta.dirname, "../../results/after");
...
async function main() {
  ...
  if (process.env.BRAINTRUST_LOG_FLUSH_CHUNK_SIZE) {
    console.log(`BRAINTRUST_LOG_FLUSH_CHUNK_SIZE=${process.env.BRAINTRUST_LOG_FLUSH_CHUNK_SIZE}`);
  }
  ...
  const result = {
    ...
    workload: WORKLOAD,
    env: {
      BRAINTRUST_LOG_FLUSH_CHUNK_SIZE: process.env.BRAINTRUST_LOG_FLUSH_CHUNK_SIZE ?? null,
    },
    rpc: {
      totalRequests,
      uploads,
